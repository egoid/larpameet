...
starry.controller("ProfileController", function($scope, $http, $localStorage, $ionicModal) {
    $scope.init = function() {
        $scope.picture = JSON.parse(localStorage.picture);
...
starry.controller("SplashController", function ($scope, $http, $cordovaOauth, $localStorage, $state, $firebaseAuth, $location) {
    ...
    $scope.init = function() {
        ...
        $http.get("https://graph.facebook.com/v2.5/me", { params: { access_token: localStorage.access_token, fields: "id,name,gender,location,picture", format: "json" }}).then(function(result) {
            ...
            console.log(error);
                    ...
                    $http.get("https://graph.facebook.com/v2.5/me/picture", {type: "large"}).then(function(response) {
